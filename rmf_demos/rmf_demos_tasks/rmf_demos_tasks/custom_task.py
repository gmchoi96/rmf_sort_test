...
class TaskRequester(Node):
    ...
    def __init__(self, argv=sys.argv):
        ...
        parser.add_argument(
            '-R', '--robot', required=True, type=str, help='Robot name'
        )
        parser.add_argument(
            '-s', '--start', required=True, type=str, help='Start waypoint'
        )
        parser.add_argument(
            '-e', '--end', required=True, type=str, help='End waypoint'
        )
        parser.add_argument(
            '-st',
            '--start_time',
        ...
        # Construct task
        msg = ApiRequest()
        msg.request_id = 'custom_task_' + str(uuid.uuid4())
        payload = {}
        ...
        activities.append(
            {
                ...
                'description': {
                    ...
                    'description': {
                        'timeout': 10,
                        'location': self.args.start
                    }
                }
            }
        )
        activities.append(
            {'category': 'go_to_place', 'description': self.args.end}
        )
